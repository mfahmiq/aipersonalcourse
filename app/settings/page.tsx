...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState({ nama_lengkap: "", email: "" })
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      let displayName = (settingsData?.nama_lengkap && settingsData.nama_lengkap !== "EMPTY") ? settingsData.nama_lengkap : authFullName || "";
      ...
      if (!settingsData) {
        await supabase
          .from("profil")
          .insert({ id: session.user.id, email: authEmail, nama_lengkap: authFullName })
      } else {
        // If nama_lengkap or email is empty, update with auth values
        if (!settingsData.nama_lengkap && authFullName) {
          await supabase
            .from("profil")
            .update({ nama_lengkap: authFullName })
            .eq("id", session.user.id)
        }
  ...
  const handleSaveAll = async () => {
    ...
    // Update name and email in profil table
    const { error: dbError } = await supabase
      .from("profil")
      .upsert({ id: userId, nama_lengkap: newName, email: newEmail }, { onConflict: "id" })

    // Update nama di Supabase Auth (user_metadata)
    let metaError = null
    if (newName !== settings.nama_lengkap) {
    ...
    if (dbError || authError || pwError || metaError) {
      ...
      setSettings((p) => ({ ...p, email: authEmail, nama_lengkap: newName }));
    ...
    <div className="min-h-[80vh] flex items-center justify-center bg-background py-8">
      <div className="w-full max-w-lg bg-background shadow-xl rounded-2xl p-8 border border-border">
        ...
        <div className="mb-6 flex flex-col items-center gap-1">
          <div className="text-lg font-semibold text-foreground">{settings.nama_lengkap || "-"}</div>
